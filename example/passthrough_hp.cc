...
#define TRACE_RW_POS(func, fx, ...) if (fs.trace_rw) \
...
static
void notify_reader(OpeningFile* fp, size_t size, off_t off, ssize_t res) {
    Inode* ino = fp->ino;
    std::unique_lock<std::mutex> lock(ino->m);
    if (ino->wpos == off) { // sequential write
        ...
        if (fs.debug) {
            fprintf(stderr, // off + res is new wpos
                "RANDwrite: ino %p fd %d, size %4zd, off %zd, wpos %zd %zd\n",
                fp->ino, fp->fd, size, off, ino->wpos, off + res);
        }
        //ino->wpos = std::max(ino->wpos, off + res);
...
#if defined(WITH_SFS_WRITE_MEM)
static void sfs_write_mem(fuse_req_t req, fuse_ino_t ino, const char *buf,
    ...
    OpeningFile* fp = (OpeningFile*)fi->fh;
    ssize_t res = pwrite(fp->fd, buf, size, off);
    fuse_reply_write(req, res);
    TRACE_RW_POS("write_mem", " %zd", off + res);
    if (res > 0 && fs.tailing_read_timeout)
        notify_reader(fp, size, off, res);
}
#endif
